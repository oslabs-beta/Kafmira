...
function ClusterContainer(props){
  ...
  useEffect(() => {
    ...
    Promise.all([totalBrokers, totalProducers])
      .then((allData) => {
  ...
  while (pCount < totalProducerList.length) {
    if (totalProducerList[0].metric.job === 'producer')
      producer.push(<Button
        variant="contained" 
        style={{ display: "flex",  width: "10vw", height: "10vh", backgroundColor: "#8d39fa", borderRadius: "50%", border: "2px solid black" }}
        >
          <p style={{ color: "white", textalign: "center", verticalalign: "middle", lineheight: "10vh" }}>Producer {pCount+1}</p>
        </Button>)
        pCount++;
  }
  ...
  const consumer = [];
  let cCount = 0;
  while (pCount < totalConsumerList.length) {
    if (totalConsumerList[0].metric.job === 'consumer') {
      consumer.push(
        <Button 
              style={{ display: 'flex', width: '10vw', height: '10vh', backgroundColor: '#089ba6', borderRadius: '50%', border: "2px solid black"}}
            >
              <p style={{color: 'white', textalign: 'center', verticalalign: 'middle', lineheight: '10vh',}}>Consumer {cCount+1}</p>
          </Button>
      )
      cCount++;
    }
